...
const Profile = (props: ProfileProps) => {
  const { data, isLoading, refetch } = api.user.getByIds.useQuery({ ids: [props.userId] });
  ...
  const isItCurrentUserPrfile = Boolean(user?.id == currentUser?.id && currentUser?.id);
        ...
        <div className="absolute top-2 right-2">
          <div className="relative">
            ...
            <Dropdown {...dropdownProps}>
              {isItCurrentUserPrfile && <DropdownItem>Zaktualizuj profil</DropdownItem>}
            </Dropdown>
          </div>
        </div>
      </div>
